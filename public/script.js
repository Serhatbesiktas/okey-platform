...
socket.on('vip_masa_kapandi', (data) => {
    if(suAnkiMasam === data.masaAdi) {
        alert("🚨 VIP oda sahibi masadan ayrıldığı için oda kapatıldı!"); masadanAyrilmaIslemi(false);
    }
});
...
window.profiliGoster = function(hedefIsim) {
    ...
    pArkadasBtn.style.display = 'none'; pDavetBtn.style.display = 'none'; pUnvanBadge.style.display = 'none'; pLigBadge.innerText = "Yükleniyor..."; pLigBadge.style.background = "#333";
    ...
    if(hedefIsim.startsWith("MİSAFİR_")) {
        ...
        pArkadasBtn.style.display = 'none'; pDavetBtn.style.display = (suAnkiMasam && isOnline) ? 'block' : 'none'; return;
    }
...
window.profilArkadasAksiyon = function() {
    ...
    } else { benimArkadaslarim.push(hedef); btn.innerText = "❌ Arkadaştan Çıkar"; btn.style.background = "#e74c3c"; ozelUyariGoster(`✅ ${hedef} arkadaş listene eklendi!`); }
...
function masadanAyrilmaIslemi(cezaUygulansinMi = false) {
    if (suAnkiMasam) {
        if (cezaUygulansinMi && masaOyunBasladiMi) {
            ...
            if (cezaMiktari > 0) {
                ...
                const cipKutu = document.getElementById('benimCipim'); cipKutu.innerText = benimAnlikCipim.toLocaleString('tr-TR'); cipKutu.style.color = "#e74c3c"; setTimeout(() => cipKutu.style.color = "", 2000);
    ...
    masayiTemizle(); document.getElementById('seatTop').innerText = "Bekleniyor..."; document.getElementById('seatLeft').innerText = "Bekleniyor..."; document.getElementById('seatRight').innerText = "Bekleniyor..."; 
...
// İŞTE DÜZELTİLMİŞ "HEMEN OYNA" BUTONU!
...
if(btnRastgeleOyna) {
    btnRastgeleOyna.addEventListener('click', () => { 
        ...
        for (const [masaAdi, koltuklar] of Object.entries(guncelMasalar)) { 
            if (masaAdi.includes('👑 VIP:')) continue; // Rastgele aramada kilitli VIP masalara girmeyi engeller
