...
// YENİ EKLENEN PROFİL FONKSİYONLARI BAŞLANGICI
window.profiliGoster = function(hedefIsim) {
    ...
    pArkadasBtn.style.display = 'none'; pDavetBtn.style.display = 'none'; pUnvan.innerText = "VIP GOLD";
    ...
    if(hedefIsim.startsWith("MİSAFİR_")) {
        pIsim.innerText = hedefIsim; pOynanan.innerText = "0"; pKazanilan.innerText = "0"; pCip.innerText = "20.000"; pUnvan.innerText = "DENEME HESABI";
    ...
    db.collection("kullanicilar").where("isim", "==", hedefIsim).get().then((querySnapshot) => {
        if(!querySnapshot.empty) {
            ...
            let oynanan = data.oynananOyun || 0; let kazanilan = data.kazanilanOyun || 0;
            let oran = oynanan > 0 ? Math.round((kazanilan / oynanan) * 100) : 0;
            pOynanan.innerText = oynanan; pKazanilan.innerText = kazanilan; kazanmaOrani.innerText = "%" + oran;
...
window.profilDavetAksiyon = function() {
    ...
    alert(`💌 ${hedef} adlı oyuncuya davet gönderildi!`);
    document.getElementById('profilEkrani').style.display = 'none';
};
// YENİ EKLENEN PROFİL FONKSİYONLARI BİTİŞİ
    ...
    db.collection("kullanicilar").where("isim", "==", nick).get().then((querySnapshot) => {
        ...
        auth.createUserWithEmailAndPassword(email, pass).then((userCredential) => {
            db.collection("kullanicilar").doc(userCredential.user.uid).set({
                ...
                oynananOyun: 0, kazanilanOyun: 0 // YENİ: İstatistik Değişkenleri
            }).then(() => { 
                auth.signOut().then(() => {
            ...
            }).catch(dbError => { console.error(dbError); alert("Veritabanı kayıt hatası."); btn.disabled = false; btn.innerText = "KAYDI TAMAMLA"; btn.style.opacity = "1"; });
        }).catch(error => { alert("Sistem Hatası: " + error.message); btn.disabled = false; btn.innerText = "KAYDI TAMAMLA"; btn.style.opacity = "1"; });
    }).catch(err => { console.error(err); alert("İsim kontrolü yapılamadı."); btn.disabled = false; btn.innerText = "KAYDI TAMAMLA"; btn.style.opacity = "1"; });
});
...
document.getElementById('btnGiris').addEventListener('click', () => {
    ...
    auth.signInWithEmailAndPassword(email, pass).then((userCredential) => {
        ...
        }).catch(dbError => { console.error("Hata:", dbError); alert("Veritabanı bağlantısında geçici bir kopukluk oldu. Sayfayı yenileyip tekrar deneyin."); btn.disabled = false; btn.innerText = "GİRİŞ YAP"; btn.style.opacity = "1"; });
    }).catch(error => { alert("Giriş Başarısız. E-posta veya şifre yanlış."); btn.disabled = false; btn.innerText = "GİRİŞ YAP"; btn.style.opacity = "1"; });
});
...
socket.on('sen_masadasin', (masaAdi) => {
    suAnkiMasam = masaAdi; lobiEkrani.style.display = 'none'; masaEkrani.style.display = 'flex';
...
socket.on('davet_geldi', (data) => {
    if(data.kime === aktifKullaniciAdi && !suAnkiMasam) { 
        document.getElementById('davetEdenIsim').innerText = data.kimden; document.getElementById('davetMasaIsim').innerText = data.masaAdi;
...
function gunlukBonusKontrol() {
    if(isMisafir) return; const bugun = new Date().toLocaleDateString('tr-TR');
...
window.gunlukKasaCevir = function() {
    if(isMisafir) return; const btn = document.getElementById('btnBonusCevir'); const gosterge = document.getElementById('dijitalGosterge');
    btn.disabled = true; btn.innerText = "KASA AÇILIYOR...";

    const oduller = [10000, 25000, 50000, 100000, 250000, 500000]; let kazanilanOdul = 10000; const sans = Math.random();
    if(sans > 0.5) kazanilanOdul = 25000; if(sans > 0.8) kazanilanOdul = 50000; if(sans > 0.9) kazanilanOdul = 100000; if(sans > 0.95) kazanilanOdul = 250000; if(sans > 0.98) kazanilanOdul = 500000;

    let animasyonSayaci = 0;
    const animasyonAraligi = setInterval(() => {
        const rastgeleSayi = Math.floor(Math.random() * 900000) + 100000;
        gosterge.innerText = rastgeleSayi.toLocaleString('tr-TR'); animasyonSayaci += 50;
        if (animasyonSayaci >= 3000) {
            clearInterval(animasyonAraligi); gosterge.innerText = kazanilanOdul.toLocaleString('tr-TR');
            gosterge.style.color = '#2ecc71'; gosterge.style.borderColor = '#2ecc71'; gosterge.style.boxShadow = 'inset 0 0 30px rgba(46, 204, 113, 0.4)'; oduluKaydet(kazanilanOdul);
        }
    }, 50);
}
...
window.magazaIslem = function(esyaId, fiyat) {
    if(isMisafir) { alert("⚠️ MİSAFİR HESAPLAR MAĞAZADAN ALIŞVERİŞ YAPAMAZ!"); return; }
    if (!benimEnvanterim.includes(esyaId)) {
        ...
        document.getElementById('benimCipim').innerText = benimAnlikCipim.toLocaleString('tr-TR'); arayuzGuncelle(); 
        ...
        if(auth.currentUser) { db.collection("kullanicilar").doc(auth.currentUser.uid).update({ cip: benimAnlikCipim, envanter: benimEnvanterim, aktifKozmetikler: aktifKozmetikler }).then(() => { alert("✅ Satın alma başarılı! Eşya otomatik olarak kuşanıldı."); }); }
    } else {
        if(aktifKozmetikler.includes(esyaId)) { aktifKozmetikler = aktifKozmetikler.filter(e => e !== esyaId); } else { aktifKozmetikler.push(esyaId); }
        arayuzGuncelle(); socket.emit('kozmetik_guncelle', { isim: aktifKullaniciAdi, kozmetikler: aktifKozmetikler }); 
...
function arayuzGuncelle() {
    const avatar = document.getElementById('vipAvatar'); const isimKutu = document.getElementById('benimAdimKutusu'); const lobiIsim = document.getElementById('lobiBenimAdim');
...
socket.on('kozmetikleri_guncelle', (data) => { globalKozmetikler = data; if(suAnkiMasam && guncelMasalar[suAnkiMasam]) { gelişmişKoltukHizala(guncelMasalar[suAnkiMasam]); } if(document.getElementById('lobiEkrani').style.display !== 'none') { socket.emit('kullanici_girisi', { isim: aktifKullaniciAdi, cip: benimAnlikCipim, kozmetikler: aktifKozmetikler }); } });
socket.on('cip_guncelle', (cip) => { benimAnlikCipim = cip; document.getElementById('benimCipim').innerText = cip.toLocaleString('tr-TR'); });
socket.on('cip_guncelle_ozel', (data) => { if(data.isim === aktifKullaniciAdi) { benimAnlikCipim = data.cip; document.getElementById('benimCipim').innerText = data.cip.toLocaleString('tr-TR'); if(auth.currentUser && !isMisafir) { db.collection("kullanicilar").doc(auth.currentUser.uid).update({ cip: data.cip }); } } });
socket.on('hata_mesaji', (mesaj) => { alert(mesaj); });

const oyunuBaslatBtn = document.getElementById('oyunuBaslatBtn'); const oyunAlanObjeleri = document.getElementById('oyunAlanObjeleri'); const kalanTasBilgi = document.getElementById('kalanTasBilgi'); const bitisAlani = document.getElementById('bitisAlani'); const ustRaf = document.getElementById('ustRaf'); const altRaf = document.getElementById('altRaf'); const masaOrtasiYazi = document.getElementById('masaOrtasiYazi'); const masaKasaBilgisi = document.getElementById('masaKasaBilgisi'); const masalarAlani = document.getElementById('masalarAlani');

let gostergeBtn = document.createElement('button'); gostergeBtn.id = 'gostergeBtn'; gostergeBtn.innerText = '⭐ GÖSTERGE YAP'; gostergeBtn.className = 'gosterge-btn'; gostergeBtn.onclick = () => { socket.emit('gosterge_goster', { masaAdi: suAnkiMasam, isim: aktifKullaniciAdi }); gostergeBtn.style.display = 'none'; }; document.getElementById('oyunAlanObjeleri').firstElementChild.appendChild(gostergeBtn);
...
socket.on('gosterge_basarili', (data) => {
    if(suAnkiMasam === data.masaAdi) {
        ...
        sesCal(sesSiraSende); document.getElementById('gostergeKutlamaMetni').innerHTML = `<strong style="color:#f2c94c;">${data.isim}</strong> gösterge yaptı!`; document.getElementById('gostergeKutlamaOdul').innerText = `+${data.odul.toLocaleString('tr-TR')} ÇİP`;
        const kutlamaEkrani = document.getElementById('gostergeKutlamaEkrani'); kutlamaEkrani.style.display = 'flex'; setTimeout(() => { kutlamaEkrani.style.display = 'none'; }, 3500);
    }
});
...
// YENİ: Firebase İstatistik Yazma İşlemi (Oyun bitiminde)
...
window.masayaOtur = function(masaAdi) { 
    ...
    if (benimAnlikCipim < bahis) { alert("⚠️ Yetersiz Bakiye! Bu masaya oturmak için en az " + bahis.toLocaleString() + " ÇİP gerekiyor."); return; }
...
document.getElementById('sohbetGonderBtn')?.addEventListener('click', () => { if(isMisafir) { alert("⚠️ MİSAFİR HESAPLAR SOHBETE MESAJ YAZAMAZ!"); return; } const input = document.getElementById('sohbetInput'); if(input.value.trim() !== '' && suAnkiMasam) { socket.emit('sohbet_mesaji', { masaAdi: suAnkiMasam, isim: aktifKullaniciAdi, mesaj: input.value, kozmetikler: aktifKozmetikler }); input.value = ''; } });
window.vipEmojiGonder = function(emoji) { if(isMisafir) { alert("⚠️ MİSAFİR HESAPLAR EMOLİ GÖNDEREMEZ!"); return; } if(suAnkiMasam) { socket.emit('vip_emoji', { masaAdi: suAnkiMasam, isim: aktifKullaniciAdi, emoji: emoji }); sohbetCekmecesi.classList.remove('acik'); } }
