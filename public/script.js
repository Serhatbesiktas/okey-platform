...
let benimKazanilanOyun = 0; // YENİ: Lig hesabı için lokal hafıza
let benimEnvanterim = []; 
...
// YENİ: Oyuncunun kazandığı maça göre LİG rengini ve ismini belirleyen motor
    ...
    if (user && !aktifKullaniciAdi) {
        db.collection("kullanicilar").doc(user.uid).get().then(doc => {
            if(doc.exists && doc.data().isim) {
                ...
                benimKazanilanOyun = doc.data().kazanilanOyun || 0; // Lig için hafızaya al
...
window.profiliGoster = function(hedefIsim) {
    ...
    const pLigBadge = document.getElementById('profilLigBadge');
    ...
    pUnvanBadge.style.display = 'none';
    ...
    db.collection("kullanicilar").where("isim", "==", hedefIsim).get().then((querySnapshot) => {
        if(!querySnapshot.empty) {
            ...
            // Hedefin Ligini Hesapla ve Yansıt
...
document.getElementById('btnMisafir').addEventListener('click', () => {
    ...
    benimAnlikCipim = 20000; benimKazanilanOyun = 0; benimEnvanterim = []; aktifKozmetikler = []; benimArkadaslarim = []; 
    ...
    auth.signInWithEmailAndPassword(email, pass).then((userCredential) => {
        ...
        db.collection("kullanicilar").doc(userCredential.user.uid).get().then(doc => {
            ...
            if(doc.exists && doc.data().isim) {
                ...
                benimKazanilanOyun = doc.data().kazanilanOyun || 0; // Girişte kazancı hafızaya al
            } else {
                kayitliNick = email.split('@')[0].toUpperCase(); benimAnlikCipim = 250000; benimKazanilanOyun = 0;
...
// YENİ: Oyun bittiğinde kendi kazancımızı yerel olarak artırıp UI güncelliyoruz
    ...
    if(suAnkiMasam === data.masaAdi) { 
        ...
        if(auth.currentUser && !isMisafir && data.kazanan && !data.kazanan.startsWith('Bot_')) {
            ...
            if(data.kazanan === aktifKullaniciAdi) {
                ...
                benimKazanilanOyun++; arayuzGuncelle(); // Lig rozetini canlı güncelle
            } else {
