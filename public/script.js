...
socket.on('vip_masa_kapandi', (data) => {
    if(suAnkiMasam === data.masaAdi) { ozelUyariGoster("🚨 VIP oda sahibi masadan ayrıldığı için oda kapatıldı!"); masadanAyrilmaIslemi(false); }
});
...
socket.on('oyun_bitti', (data) => { 
    if(suAnkiMasam === data.masaAdi) { 
        ...
        if (data.kazanan) { 
            ...
            metin.innerText = `Kazanan: ${data.kazanan}\nSebep: ${data.sebep}`; odul.innerText = `+${data.odul.toLocaleString('tr-TR')} ÇİP`; if(window.sesCal) window.sesCal(window.sesSiraSende); 
