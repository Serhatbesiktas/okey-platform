const socket = io();
let aktifKullaniciAdi = ""; let suAnkiMasam = null; let benimSiramMi = false; let guncelMasalar = {}; let gostergeHakki = false; 
let benimAnlikCipim = 0; let benimKazanilanOyun = 0; let benimEnvanterim = []; let aktifKozmetikler = []; let sonBonusTarihim = ""; let isMisafir = false; let globalKozmetikler = {}; 
let benimArkadaslarim = []; let onlineOyuncularListesi = []; let masaOyunBasladiMi = false; let cikisIcinBekleyenLogout = false; 
let suAnkiMasaVIPMi = false; let suAnkiMasaSahibi = ""; let suAnkiMasaGizliMi = false;
...
window.ozelUyariGoster = function(mesaj) { document.getElementById('uyariModalMetni').innerText = mesaj; document.getElementById('uyariModalEkrani').style.display = 'flex'; };
    ...
    if (user && !aktifKullaniciAdi) {
        db.collection("kullanicilar").doc(user.uid).get().then(doc => {
            ...
            if(doc.exists && doc.data().isim) {
                ...
                benimEnvanterim = doc.data().envanter || []; aktifKozmetikler = doc.data().aktifKozmetikler || []; sonBonusTarihim = doc.data().sonBonusTarihi || ""; benimArkadaslarim = doc.data().arkadaslar || []; benimKazanilanOyun = doc.data().kazanilanOyun || 0; 
                if(doc.data().gorevler) { benimGorevler = doc.data().gorevler; if(benimGorevler.tarih !== bugun) { benimGorevler = { kazanma: 0, mesaj: 0, gosterge: 0, tarih: bugun, alinanlar: {} }; } } else { benimGorevler = { kazanma: 0, mesaj: 0, gosterge: 0, tarih: bugun, alinanlar: {} }; }
            } else {
                kayitliNick = user.email ? user.email.split('@')[0].toUpperCase() : "OYUNCU"; benimAnlikCipim = 250000; benimKazanilanOyun = 0; benimGorevler = { kazanma: 0, mesaj: 0, gosterge: 0, tarih: bugun, alinanlar: {} };
            ...
            isMisafir = false; document.getElementById('misafirUyariBanner').style.display = 'none'; const btn = document.getElementById('btnGiris'); if(btn) { btn.disabled = false; btn.innerText = "GİRİŞ YAP"; btn.style.opacity = "1"; }
            oyunaGirisYap(kayitliNick); arayuzGuncelle(); gunlukBonusKontrol();
        }).catch(err => { ozelUyariGoster("Veritabanı yanıt vermedi. Lütfen sayfayı yenileyin."); });
    }
});
...
window.vipGizlilikDurumuDegistir = function() { if(!suAnkiMasam || !suAnkiMasaVIPMi || suAnkiMasaSahibi !== aktifKullaniciAdi) return; socket.emit('vip_masa_gizlilik_degis', { masaAdi: suAnkiMasam, isim: aktifKullaniciAdi }); };
...
window.gorevleriAc = function() { if(isMisafir) { ozelUyariGoster("⚠️ Misafir hesaplar görev yapamaz! Lütfen kayıt olun."); return; } const ekran = document.getElementById('gorevlerEkrani'); if(!ekran) return; ekran.style.display = 'flex'; renderGorevler(); }
...
function renderGorevler() {
    ...
    gorevlerData.forEach(g => {
        let yuzde = Math.min(100, (g.mevcut / g.hedef) * 100); let bittiMi = g.mevcut >= g.hedef; let alindiMi = benimGorevler.alinanlar[g.id]; let btnHtml = '';
...
window.gorevOduluAl = function(id, miktar) {
    if(benimGorevler.alinanlar[id]) return; benimGorevler.alinanlar[id] = true; gorevleriKaydet(); benimAnlikCipim += miktar; const cipKutu = document.getElementById('benimCipim');
...
window.davetMenusuAc = function() {
    ...
    const ekran = document.getElementById('arkadaslarEkrani'); const listeDiv = document.getElementById('arkadasListesiDiv'); const header = ekran.querySelector('h2');
    if(header) header.innerText = "📥 OYUNCU DAVET ET"; ekran.style.display = 'flex';
    ...
    onlineOyuncularListesi.forEach(oyuncu => {
        if(oyuncu === aktifKullaniciAdi || oyuncu.startsWith('Bot_')) return; onlineSayisi++;
        let kozmetikler = globalKozmetikler[oyuncu] || []; let isimRenk = kozmetikler.includes('atesli_isim') ? '#ff4d4d' : '#fff'; let tac = kozmetikler.includes('neon_tac') ? '👑 ' : '';
...
window.profiliGoster = function(hedefIsim) {
    if(!hedefIsim || hedefIsim === "" || hedefIsim === "Bekleniyor...") { if(suAnkiMasam) davetMenusuAc(); return; }
    ...
    let isOnline = onlineOyuncularListesi.includes(hedefIsim) || hedefIsim === aktifKullaniciAdi; let masadaMi = false;
...
window.esyaFirlatAksiyon = function(esyaIcon) {
    ...
    const hedef = document.getElementById('profilArkadasBtn').dataset.hedef; if(!hedef || !suAnkiMasam || hedef === aktifKullaniciAdi) return;
...
socket.on('esya_firlatildi', (data) => {
    ...
    ['benimAdimKutusu', 'seatRight', 'seatTop', 'seatLeft'].forEach(id => { const el = document.getElementById(id); if(!el) return; let isim = id === 'benimAdimKutusu' ? aktifKullaniciAdi : el.dataset.isim; if(isim === data.kimden) senderEl = el; if(isim === data.kime) receiverEl = el; });
    if(senderEl && receiverEl) {
        ...
        setTimeout(() => { ucanEsya.style.left = rRect.left + (rRect.width/2) - 20 + 'px'; ucanEsya.style.top = rRect.top + (rRect.height/2) - 20 + 'px'; ucanEsya.style.transform = 'scale(1.5) rotate(360deg)'; }, 50); setTimeout(() => { ucanEsya.remove(); }, 1050);
    }
});
...
window.arkadaslarMenusuAc = function() {
    ...
    const ekran = document.getElementById('arkadaslarEkrani'); const header = ekran.querySelector('h2'); if(header) header.innerText = "👥 ARKADAŞ LİSTESİ";
...
window.masayaDavetEt = function(arkadasIsmi) { 
    event.stopPropagation(); if(!suAnkiMasam) return; socket.emit('masaya_davet_et', { kimden: aktifKullaniciAdi, kime: arkadasIsmi, masaAdi: suAnkiMasam }); ozelUyariGoster(`💌 ${arkadasIsmi} adlı oyuncuya davet gönderildi!`); document.getElementById('arkadaslarEkrani').style.display = 'none'; 
}
...
socket.on('davet_geldi', (data) => {
    if(data.kime === aktifKullaniciAdi) { 
        ...
        document.getElementById('btnDavetKabul').onclick = function() { document.getElementById('davetGeldiEkrani').style.display = 'none'; if(suAnkiMasam) { socket.emit('masadan_kalk', { isim: aktifKullaniciAdi, masaAdi: suAnkiMasam }); suAnkiMasam = null; } setTimeout(() => { masayaOtur(data.masaAdi); }, 300); };
    }
});
...
function koltukStiliUygula(elementId, oyuncuIsmi) { const el = document.getElementById(elementId); if(!oyuncuIsmi) { el.innerHTML = '<span style="color:#2ecc71; font-weight:bold; font-size:13px;">➕ DAVET ET</span>'; el.style.textShadow = "none"; return; } if(oyuncuIsmi.startsWith('Bot_')) { el.innerText = oyuncuIsmi; el.style.color = "#0dcaf0"; el.style.textShadow = "none"; return; } let kozmetikler = globalKozmetikler[oyuncuIsmi] || []; let tac = kozmetikler.includes('neon_tac') ? "👑 " : ""; el.innerText = tac + oyuncuIsmi; if(kozmetikler.includes('atesli_isim')) { el.style.color = '#ff4d4d'; el.style.textShadow = '0 0 5px #ff0000'; } else { el.style.color = '#0dcaf0'; el.style.textShadow = 'none'; } }
...
if(sohbetCekmecesi) { document.getElementById('sohbetAcBtn')?.addEventListener('click', () => { sohbetCekmecesi.classList.add('acik'); }); document.getElementById('sohbetKapatBtn')?.addEventListener('click', () => { sohbetCekmecesi.classList.remove('acik'); }); document.getElementById('sohbetGonderBtn')?.addEventListener('click', () => { if(isMisafir) { ozelUyariGoster("⚠️ MİSAFİR HESAPLAR SOHBETE MESAJ YAZAMAZ!"); return; } const input = document.getElementById('sohbetInput'); if(input.value.trim() !== '' && suAnkiMasam) { socket.emit('sohbet_mesaji', { masaAdi: suAnkiMasam, isim: aktifKullaniciAdi, mesaj: input.value, kozmetikler: aktifKozmetikler }); input.value = ''; benimGorevler.mesaj++; gorevleriKaydet(); } }); }
...
window.magazaIslem = function(esyaId, fiyat) {
    ...
    if(benimEnvanterim.includes(esyaId)) {
        ...
        if(benimAnlikCipim >= fiyat) {
            ...
            socket.emit('magaza_harcamasi', { isim: aktifKullaniciAdi, yeniCip: benimAnlikCipim }); socket.emit('kozmetik_guncelle', { isim: aktifKullaniciAdi, kozmetikler: aktifKozmetikler }); ozelUyariGoster("🎉 Satın alma başarılı!"); arayuzGuncelle();
