...
// TÜM EKRAN ELEMENTLERİ EN TEPEYE ALINDI (ÇÖKME ÖNLENDİ)
...
window.profiliGoster = function(hedefIsim) {
    ...
    if(hedefIsim.startsWith("MİSAFİR_")) {
        ...
        let ligAyar = getLigRozeti(0, true);
...
socket.on('sen_masadasin', (data) => {
    ...
    suAnkiMasam = mAdi; lobiEkrani.style.display = 'none'; masaEkrani.style.display = 'flex'; document.getElementById('masaOrtasiYazi').innerText = mAdi.toUpperCase();
    ...
    if(suAnkiMasam === data.masaAdi) { 
        ...
        if(auth.currentUser && !isMisafir && data.kazanan && !data.kazanan.startsWith('Bot_')) {
            ...
            if(data.kazanan === aktifKullaniciAdi) {
                ...
                benimKazanilanOyun++; arayuzGuncelle(); 
            } else {
...
function arayuzGuncelle() {
    const avatar = document.getElementById('vipAvatar'); const isimKutu = document.getElementById('benimAdimKutusu'); 
...
function oduluKaydet(odulMiktari) {
    ...
    if(auth.currentUser) {
        db.collection("kullanicilar").doc(auth.currentUser.uid).update({ cip: benimAnlikCipim, sonBonusTarihi: bugun }).then(() => {
            document.getElementById('benimCipim').innerText = benimAnlikCipim.toLocaleString('tr-TR');
            socket.emit('kullanici_girisi', { isim: aktifKullaniciAdi, cip: benimAnlikCipim, kozmetikler: aktifKozmetikler });
            setTimeout(() => { ozelUyariGoster(`Tebrikler! Günlük kasanızdan ${odulMiktari.toLocaleString('tr-TR')} ÇİP çıktı! Yarın tekrar gelin.`); document.getElementById('gunlukBonusEkrani').style.display = 'none'; }, 1000);
        });
    }
}
...
socket.on('hatali_bitis', (mesaj) => { const tas = bitisAlani.querySelector('.okey-tasi'); if (tas) { for(let i=0; i<24; i++) { if(document.getElementById('y'+i).children.length === 0) { document.getElementById('y'+i).appendChild(tas); tas.style.position = ''; tas.style.top = ''; tas.style.left = ''; tas.style.transform = ''; tas.style.margin = ''; break; } } } setTimeout(() => { ozelUyariGoster(mesaj); }, 100); });
