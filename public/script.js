...
// YENİ: Masanın VIP Bilgi Kalkanı
...
// KAYA GİBİ SAĞLAM DOM TANIMLAMALARI (ÇÖKME ÖNLENDİ)
...
// 250K SIFIRLAMA KİLİDİ: Tarafından onaylanan kaya gibi sağlam kilit yapısı
auth.onAuthStateChanged((user) => {
    if (user && !aktifKullaniciAdi) {
        db.collection("kullanicilar").doc(user.uid).get().then(doc => {
            ...
            if(btn) { btn.disabled = false; btn.innerText = "GİRİŞ YAP"; btn.style.opacity = "1"; }
...
// YENİ: Arayüzden VIP Masa Kurma Tetikleyicisi
...
// YENİ: Canlı Masada Kilit Açma / Kapatma Aksiyonu
...
// YENİ: Canlı Kilit Senkronizasyonu
...
// YENİ: Oda Sahibi Çıkınca Masadakileri Şutlama Kalkanı
...
window.profiliGoster = function(hedefIsim) {
    ...
    const pAvatar = document.getElementById('profilAvatarAlan'); const pIsim = document.getElementById('profilIsim');
    const pOynanan = document.getElementById('profilOynanan'); const pKazanilan = document.getElementById('profilKazanilan');
    const pCip = document.getElementById('profilCip'); const kazanmaOrani = document.getElementById('profilKazanmaOrani');
    const pDurum = document.getElementById('profilDurumBadge'); const pArkadasBtn = document.getElementById('profilArkadasBtn');
    ...
    pLigBadge.innerText = "Yükleniyor..."; pLigBadge.style.background = "#333";
    ...
    if(hedefIsim.startsWith("MİSAFİR_")) {
        pIsim.innerText = hedefIsim; pOynanan.innerText = "0"; pKazanilan.innerText = "0"; pCip.innerText = "20.000"; 
        let ligAyar = getLigRozeti(0, true); pLigBadge.innerText = ligAyar.metin; pLigBadge.style.background = ligAyar.renk; pLigBadge.style.color = ligAyar.yaziRenk;
        pArkadasBtn.style.display = 'none'; pDavetBtn.style.display = (suAnkiMasam && isOnline) ? 'block' : 'none';
        return;
    }

    db.collection("kullanicilar").where("isim", "==", hedefIsim).get().then((querySnapshot) => {
        if(!querySnapshot.empty) {
            const data = querySnapshot.docs[0].data(); let kozmetikler = data.aktifKozmetikler || []; let tac = kozmetikler.includes('neon_tac') ? "👑 " : "";
            pIsim.innerHTML = tac + data.isim; pIsim.style.color = kozmetikler.includes('atesli_isim') ? '#ff4d4d' : '#fff'; pIsim.style.textShadow = kozmetikler.includes('atesli_isim') ? '0 0 8px #ff0000' : 'none';
            if (kozmetikler.includes('atesli_isim')) { pUnvanBadge.innerText = "🔥 ATEŞ USTASI"; pUnvanBadge.style.display = "inline-block"; }
            else if (kozmetikler.includes('neon_tac')) { pUnvanBadge.innerText = "👑 OKEY KRALI"; pUnvanBadge.style.display = "inline-block"; }
            if(kozmetikler.includes('altin_cerceve')) { pAvatar.style.border = '3px solid #f2c94c'; pAvatar.style.boxShadow = '0 0 15px #f2c94c'; }
            let oynanan = data.oynananOyun || 0; let kazanilan = data.kazanilanOyun || 0; let oran = oynanan > 0 ? Math.round((kazanilan / oynanan) * 100) : 0;
            pOynanan.innerText = oynanan; pKazanilan.innerText = kazanilan; kazanmaOrani.innerText = "%" + oran; pCip.innerText = (data.cip || 0).toLocaleString('tr-TR');
            let ligAyar = getLigRozeti(kazanilan, false); pLigBadge.innerText = ligAyar.metin; pLigBadge.style.background = ligAyar.renk; pLigBadge.style.color = ligAyar.yaziRenk;
        } else {
...
window.profilArkadasAksiyon = function() {
    ...
    if (benimArkadaslarim.includes(hedef)) {
        benimArkadaslarim = benimArkadaslarim.filter(name => name !== hedef); btn.innerText = "➕ Arkadaş Ekle"; btn.style.background = ""; ozelUyariGoster(`❌ ${hedef} arkadaş listenden çıkarıldı.`);
    } else {
        benimArkadaslarim.push(hedef); btn.innerText = "❌ Arkadaştan Çıkar"; btn.style.background = "#e74c3c"; ozelUyariGoster(`✅ ${hedef} arkadaş listene eklendi!`);
    }
...
function masadanAyrilmaIslemi(cezaUygulansinMi = false) {
    if (suAnkiMasam) {
        if (cezaUygulansinMi && masaOyunBasladiMi) {
            let cezaMiktari = 0;
            if (suAnkiMasam.includes('20K')) cezaMiktari = 20000; else if (suAnkiMasam.includes('50K')) cezaMiktari = 50000; else if (suAnkiMasam.includes('10K')) cezaMiktari = 10000; else cezaMiktari = 20000;

            if (cezaMiktari > 0) {
                benimAnlikCipim -= cezaMiktari; if (benimAnlikCipim < 0) benimAnlikCipim = 0;
                const cipKutu = document.getElementById('benimCipim'); cipKutu.innerText = benimAnlikCipim.toLocaleString('tr-TR');
                cipKutu.style.color = "#e74c3c"; setTimeout(() => cipKutu.style.color = "", 2000);
                ...
                document.getElementById('cezaMiktarMetni').innerText = cezaMiktari.toLocaleString('tr-TR') + " ÇİP"; document.getElementById('cezaBildirimEkrani').style.display = 'flex';
            }
        }
...
function tamamenCikisYap() {
    auth.signOut().then(() => {
        ...
        document.getElementById('authEkrani').style.display = 'flex'; document.getElementById('lobiEkrani').style.display = 'none'; document.getElementById('masaEkrani').style.display = 'none'; document.querySelector('.vip-header').style.display = 'none'; document.getElementById('cezaBildirimEkrani').style.display = 'none'; 
...
document.getElementById('btnCikisYap').addEventListener('click', (e) => {
    e.stopPropagation(); const cikisOnay = confirm("Hesabınızdan çıkış yapmak istediğinize emin misiniz?");
    if(cikisOnay) { if(suAnkiMasam && masaOyunBasladiMi) { cikisIcinBekleyenLogout = true; masadanAyrilmaIslemi(true); } else { if (suAnkiMasam) masadanAyrilmaIslemi(false); tamamenCikisYap(); } }
});
...
document.getElementById('btnMisafir').addEventListener('click', () => {
    ...
    document.getElementById('misafirUyariBanner').style.display = 'block'; oyunaGirisYap(misafirIsim); arayuzGuncelle();
});
...
document.getElementById('btnKayitTamamla').addEventListener('click', () => {
    ...
    db.collection("kullanicilar").where("isim", "==", nick).get().then((querySnapshot) => {
        ...
        auth.createUserWithEmailAndPassword(email, pass).then((userCredential) => {
            db.collection("kullanicilar").doc(userCredential.user.uid).set({ isim: nick, cip: 250000, envanter: [], aktifKozmetikler: [], sonBonusTarihi: "", arkadaslar: [], oynananOyun: 0, kazanilanOyun: 0 }).then(() => { 
                auth.signOut().then(() => { ozelUyariGoster("✅ Kayıt Başarılı! Efsanevi nickin ayarlandı.\nŞimdi lütfen e-posta ve şifrenle GİRİŞ YAP."); document.getElementById('btnGecisGiris').click(); document.getElementById('authSifre').value = ''; btn.disabled = false; btn.innerText = "KAYDI TAMAMLA"; btn.style.opacity = "1"; });
        ...
        }).catch(error => { ozelUyariGoster("Sistem Hatası: " + error.message); btn.disabled = false; btn.innerText = "KAYDI TAMAMLA"; btn.style.opacity = "1"; });
...
socket.on('sen_masadasin', (data) => {
    ...
    suAnkiMasaVIPMi = data.isVIP || false;
    suAnkiMasaSahibi = data.sahibi || "";
    suAnkiMasaGizliMi = data.gizli || false;

    lobiEkrani.style.display = 'none'; masaEkrani.style.display = 'flex'; 
    document.getElementById('masaOrtasiYazi').innerText = suAnkiMasam.toUpperCase();
    
    // Eğer VIP masaysa ve kuran bensem Canlı Gizlilik Butonunu göster
    if(suAnkiMasaVIPMi && suAnkiMasaSahibi === aktifKullaniciAdi) {
        if(btnVipGizlilikTetikle) {
            btnVipGizlilikTetikle.style.display = "block";
            btnVipGizlilikTetikle.innerText = suAnkiMasaGizliMi ? "🔓 MASAYI HERKESE AÇ" : "🔒 MASAYI KİLİTLE";
            btnVipGizlilikTetikle.style.background = suAnkiMasaGizliMi ? "#2ecc71" : "#ff33aa";
        }
    } else {
...
window.liderlikTablosunuAc = function() {
    document.getElementById('liderlikEkrani').style.display = 'flex'; const listeDiv = document.getElementById('liderlikListesi');
...
socket.on('davet_geldi', (data) => {
    if(data.kime === aktifKullaniciAdi && !suAnkiMasam) { 
        document.getElementById('davetMetni').innerHTML = `<strong style="color:#ff33aa;">${data.kimden}</strong> seni janjanlı bir VIP masaya çağırıyor patron!`;
...
// YENİ: Janjanlı VIP Oda Görünümü Sihirbazı (Lobi Render Güncellemesi)
socket.on('masalari_guncelle', (lobidekiMasalar) => { 
    ...
    for (const [masaAdi, koltuklar] of Object.entries(lobidekiMasalar)) { 
        ...
        if (masaAdi.includes('👑 VIP:')) {
            // Şov Başlasın: Neon Pembe janjanlı VIP tasarımı!
...
window.masayaOtur = function(masaAdi) { 
    let bahis = 0; if(masaAdi.includes('20K')) bahis = 20000; else if(masaAdi.includes('50K')) bahis = 50000; else if(masaAdi.includes('10K')) bahis = 10000; else bahis = 20000;
...
socket.on('cip_guncelle_ozel', (data) => { 
    if(data.isim === aktifKullaniciAdi) {
        benimAnlikCipim = data.cip; const cipKutu = document.getElementById('benimCipim');
...
function tasEkle(tasData, yuvaId) { const div = document.createElement('div'); div.className = `okey-tasi tas-${tasData.renk}`; div.innerText = tasData.sayi; div.id = data.id || tasData.id; let sonDokunma = 0; let surukleniyorMu = false; div.addEventListener('touchstart', () => { surukleniyorMu = false; }, {passive: true}); div.addEventListener('touchmove', () => { surukleniyorMu = true; }, {passive: true}); div.addEventListener('touchend', function(e) { if(surukleniyorMu) return; const simdi = new Date().getTime(); if (simdi - sonDokunma < 300) { e.preventDefault(); otomatikTasAt(this); } sonDokunma = simdi; }); div.addEventListener('dblclick', function() { otomatikTasAt(this); }); document.getElementById(yuvaId).appendChild(div); }
...
if(sohbetCekmecesi) {
    ...
    document.getElementById('sohbetGonderBtn')?.addEventListener('click', () => { if(isMisafir) { ozelUyariGoster("⚠️ MİSAFİR HESAPLAR SOHBETE MESAJ YAZAMAZ!"); return; } const input = document.getElementById('sohbetInput'); if(input.value.trim() !== '' && suAnkiMasam) { socket.emit('sohbet_mesaji', { masaAdi: suAnkiMasam, isim: aktifKullaniciAdi, mesaj: input.value, kozmetikler: aktifKozmetikler }); input.value = ''; } });
}
...
socket.on('oyun_bitti', (data) => { 
    if(suAnkiMasam === data.masaAdi) { 
        ...
        if(auth.currentUser && !isMisafir && data.kazanan && !data.kazanan.startsWith('Bot_')) {
            ...
            if(data.kazanan === aktifKullaniciAdi) { userRef.update({ oynananOyun: firebase.firestore.FieldValue.increment(1), kazanilanOyun: firebase.firestore.FieldValue.increment(1) }); benimKazanilanOyun++; arayuzGuncelle(); } 
