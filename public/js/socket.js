...
// 🔥 MASA RENDER MOTORU - BAHİS AYRIŞTIRMASI YAPILDI 🔥
socket.on('masalari_guncelle', (lobidekiMasalar) => {
    ...
    Object.entries(lobidekiMasalar).forEach(([masaAdi, koltuklar]) => {
        ...
        // MASA ADI VE BAHİS AYRIŞTIRMA (Bahis Bahis Hatası Çözümü)
        let gercekMasaAdi = masaAdi;
        let bahisMiktari = "";
        if(masaAdi.includes('(') && masaAdi.includes(')')) {
            let parcalar = masaAdi.split('(');
            gercekMasaAdi = parcalar[0].trim();
            // (20K Bahis) kısmından 'Bahis' kelimesini siliyoruz ki HTML'e eklediğimizle çakışıp çift yazmasın
            bahisMiktari = parcalar[1].replace(')', '').replace(/Bahis/gi, '').trim(); 
        }
        ...
        // Yeni şablon (Sadece HTML yerleşimi)
        masalarAlani.innerHTML += `
        <div class="masa-kart">
            <div class="kart-sol">
                <div class="masa-kart-isim">
                    <span>🎲 ${gercekMasaAdi} ${vipBadge}</span>
                    ${bahisHtml}
                </div>
            </div>
            <div class="kart-sag">
                <div class="masa-kisi-kutu">👥 ${dolu} / 4 Oyuncu</div>
                <div style="display:flex; gap:8px;">
                    ${izleBtn}
                    <button class="btn-otur ${doluRozetClass}" onclick="${action}">${txt}</button>
                </div>
            </div>
        </div>`;
