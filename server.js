...
function desteYaratVeKaristir() {
    ...
    for (let i = yeniDeste.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [yeniDeste[i], yeniDeste[j]] = [yeniDeste[j], yeniDeste[i]]; }
    return yeniDeste;
}
...
function oyunuSifirla(masaAdi, kazanan = null, odul = 0, sebep = "", okeyleBittiMi = false) {
    ...
    if(masa) {
        masa.oyunBasladi = false; masa.deste = []; masa.gosterge = null; masa.siradakiOyuncu = null;
        masa.eller = {}; masa.kasa = 0; masa.gostergeGosterildi = false;
    ...
    for (let grup of gruplar) {
        ...
        if (firstNormalIdx !== -1) {
            ...
            for (let i = firstNormalIdx + 1; i < grup.length; i++) {
                expFwdArtan = expFwdArtan === 13 ? 1 : expFwdArtan + 1; let eff = getEffectiveTile(grup[i]);
            ...
            for (let i = firstNormalIdx - 1; i >= 0; i--) {
                expBwdArtan = expBwdArtan === 1 ? 13 : expBwdArtan - 1; let eff = getEffectiveTile(grup[i]);
                if (!eff.isOkey) { if (eff.renk !== cRenk || eff.sayi !== expBwdArtan) { isSeriArtan = false; break; } }
            }
            let expFwdAzalan = cSayi;
            for (let i = firstNormalIdx + 1; i < grup.length; i++) {
                expFwdAzalan = expFwdAzalan === 1 ? 13 : expFwdAzalan - 1; let eff = getEffectiveTile(grup[i]);
                if (!eff.isOkey) { if (eff.renk !== cRenk || eff.sayi !== expFwdAzalan) { isSeriAzalan = false; break; } }
            }
            let expBwdAzalan = cSayi;
            for (let i = firstNormalIdx - 1; i >= 0; i--) {
                expBwdAzalan = expBwdAzalan === 13 ? 1 : expBwdAzalan + 1; let eff = getEffectiveTile(grup[i]);
    ...
    if(siradaki && siradaki.startsWith('Bot_')) {
        setTimeout(() => {
            ...
            if(masa.deste.length === 0) {
                io.emit('sistem_mesaji', `⚠️ ${masaAdi} masasında TAŞ BİTTİ!`);
        ...
        if(index !== -1) {
            if (masalar[m].oyunBasladi) {
                ...
                if(oyuncuCipleri[isim] !== undefined) {
                    let c = Number(oyuncuCipleri[isim]); if (isNaN(c)) c = 0; 
                ...
                masalar[m].koltuklar[index] = yeniBot; masalar[m].eller[yeniBot] = masalar[m].eller[isim]; delete masalar[m].eller[isim];
            ...
            if (masalar[m].isVIP) {
                ...
                if(masalar[m].koltuklar.every(k => k === null || k.startsWith('Bot_'))) { oyunuSifirla(m, null, 0, "Masada kimse kalmadı."); masalar[m].koltuklar = [null, null, null, null]; }
            }
...
io.on('connection', (socket) => {
  const lobiVerisi = {}; for(let m in masalar) lobiVerisi[m] = masalar[m].koltuklar;
  socket.emit('masalari_guncelle', lobiVerisi);

  socket.on('kullanici_girisi', (data) => {
      let isim = typeof data === 'object' ? data.isim : data; let dbCip = typeof data === 'object' ? data.cip : 250000;
      ...
      let c = Number(dbCip); if(isNaN(c) || c === null) c = 0; oyuncuCipleri[isim] = c; oyuncuKozmetikleri[isim] = dbKozmetikler; 
  ...
  socket.on('esya_firlat', (data) => {
      const masa = masalar[data.masaAdi]; let kimdenCip = Number(oyuncuCipleri[data.kimden]); if (isNaN(kimdenCip)) kimdenCip = 0;
      if(masa && kimdenCip >= 5000) { oyuncuCipleri[data.kimden] = Math.max(0, kimdenCip - 5000); io.emit('cip_guncelle_ozel', { isim: data.kimden, cip: oyuncuCipleri[data.kimden] }); io.emit('esya_firlatildi', data); } 
  ...
  socket.on('vip_masa_kur', (data) => {
      ...
      let miktar = Number(data.bahis); let pCip = Number(oyuncuCipleri[data.sahibi]); if(isNaN(pCip)) pCip = 0;
  ...
  socket.on('vip_masa_gizlilik_degis', (data) => {
      ...
      if (masa && masa.sahibi === data.isim) { masa.gizli = !masa.gizli; io.emit('vip_durum_guncelle', { masaAdi: data.masaAdi, gizli: masa.gizli }); const guncelLobi = {}; for(let m in masalar) guncelLobi[m] = masalar[m].koltuklar; io.emit('masalari_guncelle', guncelLobi); }
  });
  ...
  socket.on('oyunu_baslat', (masaAdi) => {
    ...
    if (masa && !masa.oyunBasladi && masa.koltuklar.includes(socket.kullaniciAdi)) {
        ...
        const guncelLobi = {}; for(let m in masalar) guncelLobi[m] = masalar[m].koltuklar; io.emit('masalari_guncelle', guncelLobi);
        masa.koltuklar.forEach(isim => { masa.kasa += masa.bahis; if(!isim.startsWith('Bot_')) { let uCip = Number(oyuncuCipleri[isim]); if(isNaN(uCip)) uCip = 0; oyuncuCipleri[isim] = Math.max(0, uCip - Number(masa.bahis)); io.emit('cip_guncelle_ozel', { isim: isim, cip: oyuncuCipleri[isim] }); } });
        masa.deste = desteYaratVeKaristir(); let secilenGosterge = masa.deste.pop(); while (secilenGosterge.renk === 'sahte') { masa.deste.unshift(secilenGosterge); secilenGosterge = masa.deste.pop(); }
        masa.gosterge = secilenGosterge; const baslayacakOyuncu = masa.koltuklar[Math.floor(Math.random() * 4)]; masa.siradakiOyuncu = baslayacakOyuncu; masa.eller = {}; 
        masa.koltuklar.forEach(oyuncuIsmi => { const kacTasAlacak = (oyuncuIsmi === baslayacakOyuncu) ? 15 : 14; const oyuncununTaslari = masa.deste.splice(0, kacTasAlacak); masa.eller[oyuncuIsmi] = oyuncununTaslari; });
        ...
        io.emit('masa_ortasi_guncelle', { masaAdi: masaAdi, kalanTas: masa.deste.length, gosterge: masa.gosterge }); io.emit('sira_guncelle', { masaAdi: masaAdi, kimde: masa.siradakiOyuncu });
  ...
  socket.on('gosterge_goster', (data) => {
      ...
      if(masa && masa.oyunBasladi && !masa.gostergeGosterildi) {
          ...
          if(hasTile) {
              ...
              io.emit('cip_guncelle_ozel', { isim: data.isim, cip: oyuncuCipleri[data.isim] }); io.emit('gosterge_basarili', { masaAdi: data.masaAdi, isim: data.isim, odul: odul });
          }
      }
  });
